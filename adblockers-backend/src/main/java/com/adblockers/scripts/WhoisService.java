...
public class WhoisService {
     ...
     * @return
     */
    public List<LegalEntity> findLegalEntitiesByUrl(List<Url> urls) {
        return urls.stream()
                .map(url -> findLegalEntitiesByUrl(url))
                .collect(Collectors.toList());
    }

    public LegalEntity findLegalEntitiesByUrl(Url url) {
        ...
        try {
            ...
            whoisResponse = WhoisService.singleOutResponse(whoisResponse, url.getDomain());
            ...
            return extractLegalEntity(url.getDomain(),
                    WhoisService.singleOutResponse(whoisResponse, url.getDomain()));
        } catch (IOException e) {
            e.printStackTrace();
        }
        return null;
...
//        return line.contains(">>> Last update of whois database");
    ...
    private static List<String> singleOutResponse(List<String> whoisResponse, String domain) {
        return Utilities.subList(
                whoisResponse,
                line -> WhoisService.isFirstResponseLine(line, domain),
                line -> WhoisService.isLastResponseLine(line));
    }
